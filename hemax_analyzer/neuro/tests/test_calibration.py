...
class TestIsotonicApplication:
    ...
    def test_apply_isotonic_passes_through_when_no_params(self, predictor):
        ...
        try:
            ...
            assert predictor._apply_isotonic("suicidal_ideation", 0.5) == 0.5
        finally:
            predictor.isotonic_params = original

    def test_apply_isotonic_for_unknown_target(self, predictor):
        assert predictor._apply_isotonic("nonexistent", 0.5) == 0.5
...
class TestSafetyCriticalCalibration:

    def test_suicidal_ideation_calibrated(self, predictor):
        assert predictor.isotonic_params is not None
        assert "suicidal_ideation" in predictor.isotonic_params

    def test_depression_severe_calibrated(self, predictor):
        assert "depression_severe" in predictor.isotonic_params

    def test_calibration_in_predict_pipeline(self, predictor, metabolic_at_risk):
