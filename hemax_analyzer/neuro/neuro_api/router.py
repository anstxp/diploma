...
async def info() -> InfoResponse:
    ...
    return InfoResponse(
        model_version=str(extras.get("model_version", "unknown")),
...
async def analyze(
    ...
    for f in ("sleep_hours_avg", "sleep_latency_min", "night_awakenings",
    ...
    logger.info(f"[{request_id}] /neuro/analyze keys={sorted(payload.keys())}")
    ...
    for r in prediction.risks:
        risks_out.append({
            ...
            "probability_ratio_vs_baseline": round(r.probability_ratio_vs_baseline, 3),
            "odds_ratio_vs_baseline": round(r.probability_ratio_vs_baseline, 3),
            "top_drivers": [
                {
                    "feature": d.feature,
                    "value": d.value,
...
async def analyze_narrative(
    ...
    logger.info(f"[{request_id}] /neuro/analyze/narrative lang={lang}")
