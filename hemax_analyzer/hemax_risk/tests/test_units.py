...
from risk_api.risk.units import normalize_lab_to_us_units, parse_lab_value
...
class TestGlucoseConversion:
    ...
    def test_mgdl_passthrough(self):
        ...
        assert normalize_lab_to_us_units("90 mg/dL", "glucose") == 90.0

    def test_mmol_to_mgdl_explicit(self):
        result = normalize_lab_to_us_units("5.0 mmol/L", "glucose")
        assert 89.0 < result < 91.0

    def test_heuristic_low_values_string_treated_as_mmol(self):
        result = normalize_lab_to_us_units("5.0", "glucose")
        assert 89.0 < result < 91.0

    def test_numeric_input_no_heuristic(self):
        assert normalize_lab_to_us_units(5.0, "glucose") == 5.0

    def test_heuristic_high_values_kept_as_mgdl(self):
        assert normalize_lab_to_us_units(90, "glucose") == 90.0
...
class TestCholesterolConversion:
    ...
    def test_ldl_same_factor(self):
        result = normalize_lab_to_us_units("3.0 mmol/L", "ldl")
        assert 115.0 < result < 117.0


class TestTriglyceridesConversion:

    def test_mmol_to_mgdl(self):
        result = normalize_lab_to_us_units("2.0 mmol/L", "trigly")
        assert 176.0 < result < 178.0
...
class TestCreatinineConversion:
    ...
    def test_heuristic_string_large_values_treated_as_umol(self):
        result = normalize_lab_to_us_units("88", "creatinine")
        assert 0.99 < result < 1.01
...
class TestBilirubinConversion:
    ...
    def test_umol_to_mgdl(self):
        result = normalize_lab_to_us_units("17 µmol/L", "bilirubin")
...
class TestHemoglobinConversion:
    ...
    def test_heuristic_string_high_values_treated_as_gl(self):
        assert normalize_lab_to_us_units("145", "hgb") == 14.5
