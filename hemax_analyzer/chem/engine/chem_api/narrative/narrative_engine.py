...
def _build_context(raw: dict) -> dict:
    ...
    def labval(code: str):
        return (labs.get(code) or {}).get("value")

    def labref(code: str, side: str):
        return ((labs.get(code) or {}).get("ref") or {}).get(side)

    def labunit(code: str):
        return (labs.get(code) or {}).get("unit")
    ...
    if sex is not None:
        ...
        if s in {"1", "m", "male", "man", "ч", "чоловік"}:
            sex_norm = "male"
        elif s in {"2", "f", "female", "woman", "ж", "жінка"}:
            sex_norm = "female"
    ...
    return {
        ...
        "glucose":    labval("glucose"),     "glucose_unit":  labunit("glucose"),
        "a1c":        labval("a1c"),         "a1c_unit":      labunit("a1c"),

        "creatinine": labval("creatinine"),  "creatinine_unit": labunit("creatinine"),
        "egfr":       labval("egfr"),
        "urea":       labval("urea"),
        "bun":        labval("bun"),

        "alt":  labval("alt"),  "ast": labval("ast"),
        "alp":  labval("alp"),  "ggt": labval("ggt"),
        "bilirubin_total":  labval("bilirubin_total"),
        "bilirubin_direct": labval("bilirubin_direct"),
        "albumin":      labval("albumin"),
        "total_protein": labval("total_protein"),

        "tchol":  labval("tchol"),
        "hdl":    labval("hdl"),
        "ldl":    labval("ldl"),
        "trigly": labval("trigly"),

        "crp": labval("crp"),

        "sodium": labval("sodium"),     "potassium": labval("potassium"),
        "chloride": labval("chloride"), "bicarbonate": labval("bicarbonate"),

        "calcium":   labval("calcium"),
        "magnesium": labval("magnesium"),
        "phosphate": labval("phosphate"),
        "vitd_25oh": labval("vitd_25oh"),

        "iron":     labval("iron"),
        "ferritin": labval("ferritin"),
        "tibc":     labval("tibc"),
        "tsat":     labval("tsat"),

        "uric_acid": labval("uric_acid"),
        "amylase":   labval("amylase"),
        "lipase":    labval("lipase"),
        "ck":        labval("ck"),
        "ldh":       labval("ldh"),
        ...
        "on_oral_anticoagulants":    bool(_clinical.get("on_oral_anticoagulants")),
        ...
        "a1c_above_target":    bool(labval("a1c") and labval("a1c") >= 7.0
                                    and _clinical.get("has_diabetes")),
        "a1c_at_target":       bool(labval("a1c") and labval("a1c") < 7.0
                                    and _clinical.get("has_diabetes")),
...
_CHROME: dict[str, dict] = {
    "uk": {
        ...
        "summary": {
            "urgent":            "У біохімії знайдено серйозне відхилення — зверніться до лікаря в найближчий час.",
            "attention_needed":  "У біохімії є знахідки, які варто обговорити з лікарем.",
            "normal":            "Біохімія крові виглядає добре — жодних значущих відхилень.",
    ...
    "en": {
        ...
        "summary": {
            "urgent":            "A serious abnormality was found in your biochemistry — see a doctor soon.",
            "attention_needed":  "There are biochemistry findings worth discussing with your doctor.",
            "normal":            "Your biochemistry results look good — no significant findings.",
...
def build_narrative(raw_result: dict, lang: str = "uk") -> NarrativeReport:
    ...
    real_findings = [c for c in selected if c["id"] not in _TERMINAL]
    if real_findings:
        selected = real_findings
    else:
        terminal_ids = {c["id"] for c in selected}
        if "incomplete" in terminal_ids and "all_normal" in terminal_ids:
            selected = [c for c in selected if c["id"] == "incomplete"]
